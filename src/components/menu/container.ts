...
export class MenuContainer extends ThemeMixin(LitElement) {
  ...
  render() {
    return html`
      <header
        class="absolute inset-x-0 z-40 w-full translate-y-[1px] bg-stone-200/60 bg-gradient-to-b backdrop-blur-md backdrop-saturate-200 xl:fixed dark:bg-zinc-950"
      >
        <div
          class="fixed inset-x-0 bottom-10 h-40 bg-gradient-to-t from-zinc-950"
        ></div>
        <div class="container grid items-center xl:grid-cols-2">
          ...
          <nav>
            <ul id="anchors" class="flex justify-end">
              ...
              <li class="flex h-10 items-center justify-center px-1 xl:h-12">
                <lang-button
