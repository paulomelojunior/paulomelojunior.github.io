...
export class TwyneProposal extends LitElement {
	...
	private renderProposalList(): unknown {
		...
		return html`
			<ul class="text-[1rem] 2xl:text-[1.25rem] leading-loose dark:text-zinc-200">
				${proposalList.map(item => html`
					<li class="flex items-center gap-4 before:text-zinc-600 text-center before:content-['✦'] before:text-[1rem]">
						${item}
					</li>
				`)}
			</ul>
	...
	private renderComponentsSection(): unknown {
		return html`
			<div id="components" class="flex relative items-start justify-center gap-4 pb-32 overflow-hidden">
				...
				<div class="container inset-y-0 absolute bg-gradient-to-t -z-10"></div>
				${this.renderComponentGrid()}
				${this.renderComponentColumn()}
			</div>
		`;
	}
